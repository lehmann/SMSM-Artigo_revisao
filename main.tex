...
\caption{Hermoupolis generated trajectories (left). A zoom in commercial center area (right)}
\label{fig:hermoupolis_groundtruth}
\end{figure*}

Table \ref{tab:hermoupolis_dataset} presents the summary of the dataset. For each profile 20 trajectories were generated, summing 140 trajectories, where 80 use the \textit{Bus} as a means of transportation and 60 use the \textit{Car} as a means of transportation.
...
\scriptsize
  ...
  \begin{tabular}{|c|c|c|c|}
  	\hline
 Profile & Stop sequence & Is worker? & Transportation Mode \\
  	\hline
P1 & Home/Store/McDonalds/Univ/Home & x & Bus \\
P2 & Home/Store/Restaurant/Univ/Home & x & Bus \\
P3 & Home/Store/McDonalds/Home & x & Bus \\
P4 & Home/Store/Restaurant/Univ/Home & x & Bus \\
P5 & Home/Store/Restaurant/Univ/Home & & Car \\
P6 & Home/McDonalds/Univ/Home & & Car \\
P7 & Home/Gym/Restaurant 2/Univ/Restaurant/Univ/Home & & Car \\
    \hline
  \end{tabular}
  \caption{Classes representing distinct paths for the ground truth}
...
To show the impact of the weight parameters on the similarity analysis, we evaluate the \emph{weight} of \emph{stops} and \emph{moves} in all experimental datasets. Table {\ref{tab:sensibility_stopmove}} shows the results for the weight varying from 0 to 1, where 0 means that the moves will be ignored and 1 means that only the moves will be considered.  As can be seen, if the influence of the \emph{moves} is ignored, i.e., weight = 0, SMSM reaches a MAP score of only $0.74$ in the Geolife dataset, $0.62$ in the Taxi dataset, and $0.79$ in the synthetic dataset, indicating a confusion between the similarity of the trajectories that are discriminated by the moves. The best result is achieved when the weight of the moves is greather than zero and less or equal than 1, achieving a MAP score of $0.95$ in the Geolife and $0.92$ in the Taxi dataset, indicating that the \emph{moves} should be considered in these datasets. In the dataset generated by Hermoupolis, if the weight of the \emph{stops} is ignored, the MAP score reaches the maximum value ($1.00$), indicating that in this dataset the \emph{move} semantic information is enough to retrieve the correct trajectories of the dataset.}
