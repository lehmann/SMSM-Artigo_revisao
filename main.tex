...
\hl{In fraud detection applications, for instance, to find a taxi fraud trajectory between two given stops, a similarity measure must be used to find the standard path that connects the stops, for than identifying the outler trajectories. A real example of an outlier taxi trajectory is shown in Figure {\ref{fig:crawdad_outlier}}, in the San Francisco dataset of the Crawdad project} \citep{epfl-mobility-20090224}. \hl{In this example, a similarity measure must consider both stops and moves to find the black trajectories as the standard path between the Airport and San Francisco central commerce to find the outlier trajectory in purple.}
...
To measure the distance between two movement elements, in this example we use the following functions for measuring the distance of each stop dimensions: 
\begin{itemize}
