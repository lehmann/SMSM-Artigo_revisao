...
\hl{The \emph{precision at recall} results with semantic trajectories shown SMSM is consistently more able to recover the relevant trajectories than others in all recall levels. The better results are explained both the \emph{move} comparison and the sequence of the \emph{stops}. As the DTW successor, DTWa remains the sequential characteristic of trajectory, but as it needs the data to be homogeneous, and as we initially claimed that \emph{stops} and \emph{moves} are intrinsically heterogeneous, it does not be able to deal with both \emph{stop} and \emph{move} of the semantic trajectories. And as the problem presented in this experiment demands to discover some behavior between \emph{stops}, DTWa was not able to distinguish trajectories traveling on distinct highways. Something similar occurs with EDR and LCSS, since both use the sequence of elements, but them need homogeneous data. MSM also does not handle heterogeneous data and does not take the sequence of elements into account either. This explains it poor results, since it ignores some important information about trajectory. CVTI and MSTP, despite of being LCSS-based measures, have poorest results because it does not take into account some information, such as geospatial data in CVTI, or it uses only the frequency of the \emph{stops}, as in MSTP.

The \emph{precision at recall} results with raw trajectories also shown SMSM as more able to recover the relevant trajectories in almost all recall levels. In this experiment, DTWa had better results, since now it uses all data in the trajectory. How DTWa tries to minimize the overall distance between two raw trajectories, now the GPS points of the (previously unknown) \emph{moves} help the measure to correctly recover the trajectories of the same class. EDR and LCSS measures, besides originally developed to raw trajectories, had poor results with raw trajectories. This occurs because of the low sampling rate in the data. As both measures compare trajectories point-by-point and them do the point matches only two points are distance less than a threshold, as highest the sampling rate, poor is the results. wDF also compares trajectories point-by-point, but it \emph{window} parameter allows it to minimize the impact of low sampled trajectories. Because of this, wDF had better results than LCSS and EDR even being a point-to-point measure. The second better results were UMS. It better results fall on it dynamic ellipses. The use of ellipses allow UMS to be more resilient over the low sampled trajectories. Even so, the SMSM results with semantic trajectories were better than all other similarity measures.}
...
\hl{As in the epfl/mobility dataset experiment, SMSM had the better results over all other measures with the semantic trajectories. Again, DTWa, LCSS, and EDR had worse results than SMSM because all of them do not take into account the \emph{move} element between the \emph{stops}. The results were not worst because them take into account the sequence of \emph{stops}. Even without considering both the \emph{move} information and the sequence of \emph{stops}, MSM had a similar precision at recall curve. This occurs in this experiment because our \emph{stops} of same semantic overlaps each other, leaving MSM to consider the \emph{stop} similarity as strongly based on it spatial dimension. If the \emph{stops} occur in a more distinct spatial locations, but remaining it semantics, MSM would say as similar two \emph{stops} spatially near, but semantically distinct, affecting it precision at recall results. MSTP and CVTI, as based-LCSS measures with constraints, suffer from the same problems of previous experiment: i) CVTI do not take into account the spatial dimension, and in this dataset the semantic dimension in \emph{stops} are closely related to spatial dimension; and ii) MSTP makes use of the frequency of the \emph{stops}, and all trajectories in this dataset are short and non-repetitive trajectories, demanding the measure to use only the spatial data of each \emph{stop}.

The precision at recall results with raw trajectories still show that SMSM outperform all other measures in all recall levels. DTWa results show that, differently from previous experiment, it has a great precision when the sampling rate of GPS is high. In the Geolife dataset, each point is collected every 2 seconds approximately. That high sampled trajectories make all it points closer to each other, leaving the DTWa to measure as similar trajectories that crossing each other, as the case in this dataset, with trajectories passing through Market sometimes crossing over trajectories passing through Starbucks. If the person spent most of the time in Market, the fact of cross trajectories of people staying in Starbuck do the trajectory similarity be higher. The same occur with LCSS and EDR. The measure wDF had a poor result because it \emph{window} parameter, when in a high sampled scenario, makes the measure evaluate much neighbor points and our dataset has the cross trajectories (as already said), increasing the probability of a point find out a point in a trajectory of other class closer than trajectories from same class. UMS measure, the second most precise measure in previous experiment, was the fourth more precise measure. Again, it was because the high sampling rate. UMS was designed to handle with low sampled trajectories by creating it ellipses around points. In a high sampled scenario, it ellipses tends to be small, not overlapping the points of the other trajectory.}
